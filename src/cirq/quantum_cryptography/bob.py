...
class Bob:
    ...
    def decode(self, qubits):
        ...
        yield cirq.measure(*qubits, key='bob')

    def measure_qubits(self, circuit):
        # qubitを測定する
        simulator = cirq.google.XmonSimulator()
        result = simulator.run(circuit)
        self._result_bits = [int(bit) for bit in result.measurements['bob'][0]]
    ...
    # def measure_qubits(self, circuit, qubit, othera):
    #     circuit.append(cirq.measure(qubit, key='bob'))
    #     # qubitを測定する
    #     simulator = cirq.google.XmonSimulator()
    #     result = simulator.run(circuit)
    #     return result.measurements['bob'][0]
